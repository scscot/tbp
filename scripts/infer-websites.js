#!/usr/bin/env node
/**
 * Infer websites for ALL contacts based on email domain
 *
 ...
 *
 * Works with ALL sources (calbar, flbar, gabar, mibar, ilbar, etc.)
 *
 * Usage: node scripts/infer-websites.js
 *
 * Options:
...
<body>
    <div class="header">
        <h2 style="margin: 0;">🌐 Website Inference Report (All Sources)</h2>
    ...
    <div class="footer">
        <p>Generated by PreIntake.ai Website Inference Tool</p>
    </div>
</body>
</html>`;

    const plainText = `
Website Inference Report (All Sources)
 ...
 */
async function main() {
    console.log('🌐 Website Inference Tool (All Sources)');
    console.log('========================================\n');
    ...
    // Query ALL contacts without websites AND not already checked
    // Works with any source (calbar, flbar, gabar, mibar, ilbar, nebar, msbar, ohbar, etc.)
    const query = db.collection('preintake_emails')
        .where('website', '==', '')
        .where('domainChecked', '==', false);

    const snapshot = await query.get();

    // Also get contacts where domainChecked doesn't exist yet (legacy records)
    const legacyQuery = db.collection('preintake_emails')
        .where('website', '==', '');
    const legacySnapshot = await legacyQuery.get();

    // Filter legacy to only unchecked
    const uncheckedLegacy = legacySnapshot.docs.filter(doc => {
        const data = doc.data();
    ...
    const sourceCounts = {};
    uncheckedContacts.forEach(doc => {
        const source = doc.data().source || 'unknown';
        sourceCounts[source] = (sourceCounts[source] || 0) + 1;
    });

    console.log('📊 Source breakdown (contacts without websites):');
