#!/usr/bin/env node
/**
 * PreIntake.ai Email Campaign Sender - GMAIL ONLY
 *
 * VARIANT: This is the Gmail-specific version using law.preintake.ai domain.
 * ONLY sends to contacts with gmail.com/googlemail.com email addresses.
 *
 * The main campaign (send-preintake-campaign.js) uses legal.preintake.ai and
 * excludes Gmail during domain warming. This script uses law.preintake.ai
 * (which has better Gmail inbox placement) to target Gmail contacts specifically.
 *
 ...
 *   MAILGUN_API_KEY - Mailgun API key for law.preintake.ai
 ...
 *   MAILGUN_API_KEY=xxx node scripts/send-preintake-campaign-gmail.js
 *
 *   # Test mode (sends to test email, doesn't update Firestore)
 *   TEST_EMAIL=test@example.com MAILGUN_API_KEY=xxx node scripts/send-preintake-campaign-gmail.js
 */
 ...
 * This allows manual control of Gmail campaign batch sizes via Firestore
 * Reads from DEFAULT database config/emailCampaign.preintakeGmailBatchSize field
 */
async function getDynamicBatchSize() {
  try {
    const configDoc = await configDb.collection('config').doc('emailCampaign').get();
    if (configDoc.exists && configDoc.data().preintakeGmailBatchSize) {
      const firestoreBatchSize = configDoc.data().preintakeGmailBatchSize;
      console.log(`📊 Using Firestore Gmail batch size: ${firestoreBatchSize}`);
...
// Gmail-only campaign: This script ONLY sends to Gmail addresses
...
/**
 * Check if an email address is from Gmail (Gmail-only campaign targets these)
 */
...
async function runCampaign() {
    ...
    console.log(`   Gmail Only Mode: ${GMAIL_ONLY ? 'ENABLED (only Gmail contacts)' : 'DISABLED'}`);
    if (TEST_EMAIL) {
    ...
    // If specific document ID is provided, use that instead of querying
        ...
        // Gmail-only filter: ONLY include Gmail contacts
        if (GMAIL_ONLY) {
            ...
            console.log(`📧 Gmail-only mode: ${mergedDocs.length} Gmail contacts (filtered out ${filtered} non-Gmail)`);
        }
