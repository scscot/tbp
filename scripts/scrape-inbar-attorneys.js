#!/usr/bin/env node
/**
 * Indiana Bar Attorney Scraper (Puppeteer + vCard API)
 *
 * Scrapes attorney contact information from the Indiana State Bar Association website
 * (inbar.reliaguide.com) and imports into preintake_emails collection.
 *
 * NOTE: Indiana's ReliaGuide site does NOT expose practice areas on attorney cards
 * or support category URL filtering. We scrape ALL attorneys without practice area
 * filtering - contact data is still valuable for outreach.
 *
 * Strategy:
 ...
 *
 * Usage:
 *   node scripts/scrape-inbar-attorneys.js
 *
...
const BASE_URL = 'https://inbar.reliaguide.com';
...
async function loadProgress() {
    ...
    try {
        const progressRef = db.collection('preintake_scrape_progress').doc('inbar');
...
async function scrapeAllAttorneys() {
    console.log('Indiana Bar Attorney Scraper');
    console.log('='.repeat(60));
    console.log('NOTE: Indiana does NOT expose practice areas');
    console.log('Scraping ALL attorneys without practice area filtering');
    console.log();
    ...
    try {
        ...
        while (hasMorePages && insertedThisRun < MAX_ATTORNEYS) {
            currentPage++;
    ...
    console.log(`   Total Indiana Bar in DB: ${totalInDb}`);
    ...
    const summaryPath = path.join(__dirname, 'inbar-scrape-summary.json');
...
async function sendNotificationEmail(summary) {
    ...
    const subject = DRY_RUN
        ? '[DRY RUN] Indiana Bar Scraper Complete'
        : 'Indiana Bar Scraper Complete';

    const text = `
Indiana Bar Attorney Scraper Summary
====================================

NOTE: Indiana does not expose practice areas - all attorneys scraped without filtering

Pages Scraped: ${summary.pagesScraped}
Inserted This Run: ${summary.insertedThisRun}
Total Indiana Bar in DB: ${summary.totalInDb}
