...
const RESULTS_PER_PAGE = 40; // ReliaGuide shows ~40 per page
...
async function extractProfileIds(page) {
    return await page.evaluate(() => {
        ...
        // Also look for profile links (IL uses IL- prefix in URLs)
        const profileLinks = document.querySelectorAll('a[href*="/lawyer/"]');
        profileLinks.forEach(link => {
            const match = link.href.match(/lawyer\/\d+-\w+-\w+-\w+-(\d+)/);
    ...
    try {
        ...
        for (let i = 0; i < allProfileIds.length && i < MAX_ATTORNEYS; i++) {
            ...
            try {
                ...
                // State handling - default to IL for Illinois Bar members
...
async function main() {
    console.log('Illinois Bar Attorney Scraper (Puppeteer + vCard)');
    console.log('='.repeat(60));
    console.log('Strategy: Puppeteer browser + vCard API for reliable email extraction');
    ...
    console.log('Loading existing IlBar profile IDs...');
    ...
    try {
        ...
        for (const category of validCategories) {
            ...
            // Check MAX_COMBOS limit
            if (categoriesProcessed >= MAX_COMBOS) {
                console.log(`\nReached MAX_COMBOS limit (${MAX_COMBOS})`);
                break;
            }

            // Check MAX_ATTORNEYS limit
