#!/usr/bin/env node
/**
 * Team Build Pro - App Update Email Script (PROSPECTS ONLY)
 *
 * Sends a follow-up email to Team Build Pro PROSPECTS (role=user)
 * announcing the reduced qualification milestones (3 direct + 12 total).
 *
 * Usage:
 *   node scripts/send-app-update-email-prospects.js --dry-run   # Preview without sending
 *   node scripts/send-app-update-email-prospects.js --test      # Send test email to yourself
 *   node scripts/send-app-update-email-prospects.js --send      # Send to all prospects
 *   node scripts/send-app-update-email-prospects.js --stats     # Show user counts
 *
 * Environment:
...
const UTM_CAMPAIGN = 'qualification_update_prospects';
...
// Localized content - Qualification milestone update for PROSPECTS
const TRANSLATIONS = {
  en: {
    ...
    whatThisMeans: (company) => `This means you'll receive your invitation to officially join <strong>${company}</strong> even sooner. And best of all, your Team Build Pro team members will follow you into ${company} as they become qualified.`,
    benefit: "Fewer hurdles. Faster momentum. You'll be ready to hit the ground running.",
  ...
  es: {
    ...
    whatThisMeans: (company) => `Esto significa que recibirás tu invitación para unirte oficialmente a <strong>${company}</strong> aún más pronto. Y lo mejor de todo, tus miembros del equipo de Team Build Pro te seguirán a ${company} a medida que califiquen.`,
    benefit: 'Menos obstáculos. Más impulso. Estarás listo para empezar con todo.',
  ...
  pt: {
    ...
    whatThisMeans: (company) => `Isso significa que você receberá seu convite para se juntar oficialmente a <strong>${company}</strong> ainda mais cedo. E o melhor de tudo, seus membros da equipe Team Build Pro seguirão você para ${company} à medida que se qualificarem.`,
    benefit: 'Menos obstáculos. Mais impulso. Você estará pronto para começar com tudo.',
  ...
  de: {
    ...
    whatThisMeans: (company) => `Das bedeutet, dass du deine Einladung, dich offiziell <strong>${company}</strong> anzuschließen, noch früher erhältst. Und das Beste: Deine Team Build Pro Teammitglieder werden dir zu ${company} folgen, sobald sie sich qualifizieren.`,
    benefit: 'Weniger Hürden. Schnellerer Schwung. Du wirst bereit sein, durchzustarten.',
...
// =============================================================================
// ADMIN SETTINGS CACHE (for biz_opp lookup)
// =============================================================================
...
// =============================================================================
// EMAIL HTML TEMPLATE (Qualification Update - Prospects)
// =============================================================================
...
async function getExistingUsers() {
  ...
  for (const doc of usersSnapshot.docs) {
    ...
    if (data.email &&
      ...
      // Fetch biz_opp from upline_admin's admin_settings
      const bizOpp = await getAdminBizOpp(data.upline_admin);
...
async function checkAlreadySent(uid) {
  // Check if we already sent this qualification update email to this prospect
  const doc = await db.collection('app_update_emails_qualification_prospects').doc(uid).get();
  return doc.exists;
}

async function markAsSent(uid, email, messageId, lang, bizOpp) {
  await db.collection('app_update_emails_qualification_prospects').doc(uid).set({
    email: email,
    messageId: messageId,
...
async function showStats() {
  console.log('\n📊 Prospect Statistics (role=user)\n');

  const users = await getExistingUsers();
  console.log(`Total eligible prospects: ${users.length}`);

  // Count already sent
  const sentSnapshot = await db.collection('app_update_emails_qualification_prospects').get();
...
async function sendTestEmail(forceLang = null, forceCompany = null) {
  const langNames = { en: 'English', es: 'Spanish', pt: 'Portuguese', de: 'German' };
  const langDisplay = forceLang ? ` in ${langNames[forceLang] || forceLang}` : '';
  const company = forceCompany || 'Amway';
  console.log(`\n📧 Sending Test Email${langDisplay} (Company: ${company})\n`);
  ...
  try {
    const result = await sendEmailViaMailgun(testUser, company);
...
async function dryRun() {
  console.log('\n🔍 Dry Run - Preview Only (Prospects)\n');

  const users = await getExistingUsers();
  console.log(`Found ${users.length} eligible prospects\n`);
  ...
  for (const user of users.slice(0, 20)) { // Check first 20
    ...
    if (sent) {
      ...
      console.log(`Would send to: ${user.email} (${user.firstName || 'No name'}) [${lang}] - Company: ${company}`);
    }
  }

  if (users.length > 20) {
    console.log(`... and ${users.length - 20} more prospects`);
  }
...
async function sendToAllProspects() {
  console.log('\n🚀 Sending Qualification Update Emails to Prospects\n');
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    try {
      const lang = detectLanguage(user);
      const company = user.bizOpp || 'business';
      const result = await sendEmailViaMailgun(user, company);
      await markAsSent(user.uid, user.email, result.messageId, lang, company);
      sent++;
      console.log(`✅ [${sent}/${users.length}] Sent to ${user.email} (${lang}) - ${company}`);
...
async function main() {
  ...
  if (args.includes('--help') || args.length === 0) {
    console.log(`
Team Build Pro - Qualification Update Email Script (PROSPECTS)

Usage:
  node scripts/send-app-update-email-prospects.js --dry-run              Preview without sending
  node scripts/send-app-update-email-prospects.js --test                 Send test email to yourself
  node scripts/send-app-update-email-prospects.js --test --lang=es       Send test in Spanish
  node scripts/send-app-update-email-prospects.js --test --company=Amway Send test with specific company
  node scripts/send-app-update-email-prospects.js --send                 Send to all prospects
  node scripts/send-app-update-email-prospects.js --stats                Show prospect counts
`);
    process.exit(0);
  }
  ...
  try {
    if (args.includes('--stats')) {
      ...
      const forceCompany = companyArg ? companyArg.split('=')[1] : null;
      await sendTestEmail(forceLang, forceCompany);
    } else if (args.includes('--dry-run')) {
      await dryRun();
    } else if (args.includes('--send')) {
      // Confirmation prompt
      console.log('\n⚠️  WARNING: This will send qualification update emails to ALL prospects!\n');
      console.log('Press Ctrl+C within 5 seconds to cancel...\n');
      await sleep(5000);
      await sendToAllProspects();
    } else {
